            ...
            <nav className="hidden md:flex items-center space-x-6">
              ...
              {currentUser && (
                ...
                >
                  Cart
                  {itemCount > 0 && (
                    <span className="ml-2 bg-white text-black text-xs rounded-sm h-5 w-5 flex items-center justify-center">
                      {itemCount}
                    </span>
                  )}
                </Link>
              ...
              {currentUser ? (
                <div className="relative">
                  ...
                  {isMenuOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-gray-800 border border-gray-700 rounded-sm shadow-lg py-1 z-10">
                      <Link
                        to="/profile"
                ...
                <Link
                  to="/login"
                  className="bg-white text-black px-4 py-2 rounded-sm hover:bg-gray-200 transition-colors font-medium font-inter"
                >
                  Sign in
                </Link>
              )}
            ...
            <div className="md:hidden mt-4 border-t border-gray-800 pt-4">
              <form onSubmit={handleSearch} className="mb-4">
                <div className="flex items-center">
                  <input
                    type="text"
                    placeholder="Search products..."
                    className="w-full px-4 py-2 bg-black border border-gray-700 rounded-sm focus:outline-none focus:border-white text-white"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
                  <button type="submit" className="bg-white text-black px-4 py-2 rounded-sm hover:bg-gray-200">
                    <Search className="h-5 w-5" />
                  </button>
                </div>
              </form>
              <div className="flex flex-col space-y-4">
                ...
                {currentUser ? (
                  ...
                  <Link
                    to="/login"
                    className="bg-white text-black px-4 py-2 rounded-sm hover:bg-gray-200 inline-block font-inter"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    Sign in
                  </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <div className="flex mt-6 space-x-4">
                <a
                  href="#"
                  className="h-10 w-10 bg-black rounded-sm flex items-center justify-center text-gray-400 hover:text-white transition-colors"
                >
